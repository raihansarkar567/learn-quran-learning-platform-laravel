@extends('layouts.app',['pageTitle' => __('alphabet Add')])
@section('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <div class="header-action">
                <a href="{{ url('/admin/alphabet/create') }}" role="button" class="btn btn-primary"><i class="fa fa-plus mr-2"></i>Add</a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">alphabet List</h3>
                    ...
                    <table class="table table-striped table-vcenter table-hover mb-0 text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="font-size: 4em;">{{$item->letter}}</td>
                                <td>{{$item->name}}</td>
                                <td>{{$item->getMakharij->name}}</td>
                                <td>
                                ...
                                <td>
                                    {{-- sound play btn--}}
                                    <a onClick="togglePlay{{$item->id}}()" title="alphabet sound">
                                    ...
                                    <a href="{{ url('/admin/alphabet/' . $item->id) }}" title="View alphabet"><button
                                        class="btn btn-info btn-sm"><i class="fa fa-eye"
                                            aria-hidden="true"></i></button></a>

                                    <a href="{{ url('/admin/alphabet/' . $item->id . '/edit') }}" title="Edit Class"><button
                                class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                            </button></a>
                        <form method="POST" id="deleteButton{{$item->id}}"
                            action="{{ url('/admin/alphabet' . '/' . $item->id) }}" accept-charset="UTF-8"
